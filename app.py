...
def analyze_sentiment(text):
    ...
    if analysis.sentiment.polarity > 0:
        ...
        return 'Sad'
def clean_text(text):
    return ' '.join(re.sub('(@[A-Za-z0-9]+)|([^0-9A-Za-z \t])|(\w+:\/\/\S+)|([RT])', ' ', text.lower()).split())
...
def uploadtext():
    ...
    return render_template('test.html',music=sent,emotion=sent,
                           sentence=mood(sent), activity=activities(sent),link=provide_url(sent),image=sent)
